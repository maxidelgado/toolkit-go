...
// The library transforms the proxy event into an HTTP ctxhelper and then
...
func (f *FiberLambda) proxyInternal(req *http.Request, err error) (events.APIGatewayProxyResponse, error) {
	...
	if err != nil {
		return core.GatewayTimeout(), core.NewLoggedError("Could not convert proxy event to ctxhelper: %v", err)
	}
...
func (f *FiberLambda) adaptor(w http.ResponseWriter, r *http.Request) {
	// New fasthttp ctxhelper
	var req fasthttp.Request
	// Convert net/http -> fasthttp ctxhelper
